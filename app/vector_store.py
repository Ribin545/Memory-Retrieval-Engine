"""
Memory Retrieval Engine — Vector Store

Local ChromaDB wrapper for memory embeddings and retrieval.

Design:
- Persistent ChromaDB client stored in the legacy protected Chroma path.
...
from app.paths import (
    PROTECTED_LEGACY_CHROMA_DIR,
    LEGACY_MEMORIES_PATH,
    LEGACY_MEMORY_COLLECTION,
)

CHROMA_DIR = PROTECTED_LEGACY_CHROMA_DIR
MEMORIES_PATH = LEGACY_MEMORIES_PATH
DEFAULT_COLLECTION = LEGACY_MEMORY_COLLECTION
...
def get_collection(client, name: str = "legacy_memories"):
    """
    ...
    for i in range(0, total, batch_size):
        ...
        for m in batch:
            ...
            meta = {
                ...
                "action_frame_type": str((m.get("action_frame") or {}).get("frame_type", "") or ""),
                "action_root_verb": str((m.get("action_frame") or {}).get("root_verb_lemma", "") or (m.get("action_frame") or {}).get("root_verb", "") or ""),
                "action_secondary_verb": str((m.get("action_frame") or {}).get("secondary_verb_lemma", "") or (m.get("action_frame") or {}).get("secondary_verb", "") or ""),
                "action_object_text": str((m.get("action_frame") or {}).get("object_text", "") or ""),
                "action_context_text": str((m.get("action_frame") or {}).get("context_text", "") or ""),
                "action_time_context": str((m.get("action_frame") or {}).get("time_context", "") or ""),
                "action_emotion_text": str((m.get("action_frame") or {}).get("emotion_text", "") or ""),
                "query_affordances_json": json.dumps(m.get("query_affordances", []), ensure_ascii=False),
                # Topic tags
